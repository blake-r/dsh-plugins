...
// SHORT form: first sentence of the original description + `Requires ...` /
// `Optionally ...` from the parameters (no full recursive schema, no
// `Use tool_call \`<name>\` to` prefix). The full `parameters` schema stays in
// the tool registry; only the short lines are rendered.
//
...
function typeOf(node) {
  ...
  if (Array.isArray(node.type)) return node.type.filter((t) => t !== "null").join("|") || "any";
  if (Array.isArray(node.oneOf)) return node.oneOf.filter((n) => n !== null).map(typeOf).join("|") || "any";
  if (Array.isArray(node.anyOf)) return node.anyOf.filter((n) => n !== null).map(typeOf).join("|") || "any";
  if (node.enum !== undefined) return node.enum.map((v) => `"${String(v)}"`).join("|");
  if (node.items !== undefined) return `${typeOf(node.items)}[]`;
  return "any";
}

function reqType(node) {
  if (node !== undefined && node !== null && node.type === "array") {
...
// `Requires 'name' type, ...` from required params only; '' when none.
function requiresLine(parameters) {
  if (parameters === undefined || parameters === null || typeof parameters !== "object") return "";
  const required = Array.isArray(parameters.required) ? parameters.required : [];
  const props = parameters.properties ?? {};
  const items = required
    .filter((key) => typeof key === "string" && props[key] !== undefined)
    .map((key) => `'${key}' ${reqType(props[key])}`);
  if (items.length === 0) return "";
  return `Requires ${items.join(", ")}.`;
}

// `Optionally 'name' type, ...` from optional params only; '' when none.
function optionallyLine(parameters) {
  if (parameters === undefined || parameters === null || typeof parameters !== "object") return "";
  ...
  const items = Object.keys(props)
    ...
    .map((key) => `'${key}' ${reqType(props[key])}`);
  if (items.length === 0) return "";
  return `Optionally ${items.join(", ")}.`;
}

// Short description line for a tool: first sentence + Requires/Optionally.
function shortLine(tool) {
  const desc = firstSentence(tool.description);
  const req = requiresLine(tool.parameters);
  const opt = optionallyLine(tool.parameters);
  return [desc, req, opt].filter(Boolean).join(" ");
}
...
function apply(ctx) {
  ...
  ctx.on("system-prompt/assemble", async (assembly, context, next) => {
    ...
    for (const tool of all) {
      ...
      const full = [catalogDesc, req, opt].filter(Boolean).join(" ");
